...
export declare class MapworksMap {
  ...
  //
  // Map ready event.
  // Fired after the map is loaded and is ready to be used.
  //
  public on(eventType: 'ready', callback: () => void): MapworksMap;

  /**
   * Registers a callback to be executed after the 'accessToken:change' event is fired
   * @param eventType Event triggered when access token is changed.
   * @listen tokenType The new token type 
   * @param accessToken The new token type
   */
